...
3. **Run the pipeline.** `python3 scripts/pipeline_green.py work/<name> <name>`. It keys, finds the walk cycle, reconstructs every frame with pixel-art-fixer at one cell size, removes edge specks and packages frames, sheets, JSON and a loop GIF. The grid is anchored to the feet baseline by default (`ANCHOR=feet`), which stops the sprite popping up and down by whole cells. If the intake or the contact sheet shows the character leaving the ground (jump, hover, flight), run with `ANCHOR=none` and tell the user why.
